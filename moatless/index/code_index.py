...
class CodeIndex:
    ...
    def find_related_code(
        ...
        """
        查找与指定代码片段相关的代码，包括引用关系、继承关系和调用关系。
        
        Args:
            code_snippet: 要查找相关内容的代码片段
            problem_description: 问题描述，用于评估相关性
            file_pattern: 可选的文件模式过滤
            max_results: 最大结果数量
            
        Returns:
            SearchCodeResponse: 搜索结果
        """
        # 首先尝试找到完全匹配
        exact_matches = []
        if file_pattern:
            ...
            for file_path in matching_files:
                file_matches = self._file_repo.find_exact_matches_in_file(file_path, code_snippet)
                exact_matches.extend(file_matches)
        else:
            exact_matches = self._file_repo.find_exact_matches(code_snippet)
            
        if not exact_matches:
            # 如果没有精确匹配，使用语义搜索
            logger.info(f"find_related_code: 找不到精确匹配，使用语义搜索")
        ...
        logger.info(f"find_related_code: 找到 {len(exact_matches)} 个精确匹配")
        # 使用精确匹配构建搜索响应
        hits = []
        for file_path, line_num in exact_matches[:max_results]:
            file = self._file_repo.get_file(file_path)
            ...
            for span in spans:
                hit.add_span(span.span_id, 0)
                
            if hit.spans:
                hits.append(hit)
        
        return SearchCodeResponse(
            message=f"找到 {len(hits)} 个与代码片段相关的匹配项",
            hits=hits
        )
