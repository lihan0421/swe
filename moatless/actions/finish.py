    ...
    def get_few_shot_examples(cls) -> List[FewShotExample]:
        return [
            FewShotExample.create(
                user_input="The last executed action fixed a bug in the authentication middleware in django/auth/middleware.py where unauthenticated users were inadvertently granted access due to a misconfigured session check.",
                action=FinishArgs(
                    thoughts="I have corrected the session validation logic in the authentication middleware.",
                    finish_reason="The bug in the authentication middleware has been resolved by implementing proper session checks.",
                ),
            ),
            FewShotExample.create(
                user_input="The last executed action enhanced error reporting in django/core/handlers/exception.py, ensuring that stack traces include detailed context for easier debugging.",
                action=FinishArgs(
                    thoughts="I enriched the exception logging to capture more contextual data and improved stack trace clarity.",
                    finish_reason="Developers now receive comprehensive error reports, significantly simplifying the debugging process. Simulated error scenarios confirm the improvements.",
                ),
            ),
            # FewShotExample.create(
            #     user_input="The bug in the data validation function has been fixed by removing the incorrect validation logic and adding a proper implementation.",
            #     action=FinishArgs(
            #         thoughts="I identified the issue in the validation function where it was using an incorrect comparison operator (== instead of !=) and also failing to handle edge cases with empty values. After removing the problematic code using CodeDeleteId, I added the corrected implementation with CodeInsert that properly validates all edge cases and includes comprehensive error handling. All tests are now passing.",
            #         finish_reason="The data validation bug has been completely fixed through a two-step process: (1) first removing the incorrect validation logic using CodeDeleteId and (2) then inserting the proper implementation with CodeInsert. The new code correctly handles all edge cases that were previously failing, and I've verified this with all test cases now passing successfully.",
            #     ),
            # ),
        ]